...
function backLinker(e){
	var target = $(e).parents('article').attr('id'),
		theGoods = content[target].pop(); 
	$(e).parent().empty().append(theGoods);
}
function searcher(input, contentType){
	...
	content[contentType + 'Article'].push($('#' + contentType + 'Article').children().detach());
	console.log(content);
...
// start background
	...
	$('body').on('click', '#playPause', function() {
		...
		if (icon.hasClass("fa-play")){
			icon.removeClass('fa-play').addClass('fa-equals fa-rotate-90');
		} else if (icon.hasClass('fa-equals')){
			icon.addClass('fa-play').removeClass('fa-equals fa-rotate-90');
		}
	});
});
